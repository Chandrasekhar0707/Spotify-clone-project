...
async function main() {
    ...
    // when a track ends, advance to next in the current playlist
    currentSong.addEventListener('ended', () => {
        if (!Array.isArray(songs) || songs.length === 0) return;

        // get the filename from the audio src and decode it to match entries in `songs`
        const lastSegment = currentSong.src.split('/').slice(-1)[0] || '';
        const currentTrackName = decodeURIComponent(lastSegment);

        // try direct lookup, then fallback to a decoded comparison if needed
        let index = songs.indexOf(currentTrackName);
        if (index === -1) {
            index = songs.findIndex(s => decodeURIComponent(s) === currentTrackName || s === currentTrackName);
        }
        ...
        if (index >= 0 && index + 1 < songs.length) {
            ...
            // reached end — wrap to first track
            currentTrackIndex = 0;
            playMusic(songs[0]);
        }
    })
